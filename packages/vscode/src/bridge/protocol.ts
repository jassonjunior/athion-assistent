/**
 * Protocolo JSON-RPC 2.0 para comunicação Extension ↔ Core (via stdio).
 *
 * A extensão spawna `bun serve --mode=stdio` como child process.
 * Comunicação bidirecional via stdin/stdout com JSON delimitado por \n.
 *
 * Fluxo:
 *   Extension → stdin → Bun Server (request)
 *   Bun Server → stdout → Extension (response + notifications)
 */
...
/** Methods the extension can call on the server */
